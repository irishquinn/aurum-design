...
class WR_Nitro
{
    ...
    public static function initialize()
    {
        ...
        add_action('wp', [__CLASS__, 'get_options']);

        // Register necessary actions.
        add_action('admin_menu', [__CLASS__, 'admin_menu']);

        // Add custom css to admin
        add_action('admin_head', [__CLASS__, 'admin_custom_css']);
    ...
    public static function get_header()
    {
        ...
        $header_data = !empty($header_data['post_content']) ? (is_serialized($header_data['post_content']) ? unserialize($header_data['post_content']) : json_decode($header_data['post_content'], true)) : [];
     ...
     * @return  boolean
     */
    public static function check_upload($file, $options = [])
    {
        // Prepare options.
        $options = wp_parse_args($options, [
            'null_byte' => true,  // Check for null byte in file name.
            'forbidden_extensions' => [ // Check if file extension contains forbidden string (e.g. php matched .php, .xxx.php, .php.xxx and so on).
            ...
            'shorttag_extensions' => [ // File extensions that need to check if file content contains short open tag.
            ...
            'fobidden_ext_extensions' => [ // File extensions that need to check if file content contains forbidden extensions.
        ...
        if ($options['php_tag_in_content'] || $options['shorttag_in_content'] || ($options['fobidden_ext_in_content'] && !empty($options['forbidden_extensions']))) {
            ...
            if ($options['shorttag_in_content']) {
                ...
                if (empty($suspicious_exts)) {
                    $suspicious_exts = ['inc', 'phps', 'class', 'php3', 'php4', 'txt', 'dat', 'tpl', 'tmpl'];
                }
            ...
            if ($options['fobidden_ext_in_content'] && !empty($options['forbidden_extensions'])) {
                ...
                if (empty($suspicious_exts)) {
                    $suspicious_exts = ['zip', 'rar', 'tar', 'gz', 'tgz', 'bz2', 'tbz'];
                }
    ...
    public static function admin_custom_css()
    {
        echo '
			<style>
				...
				.rs-update-notice-wrap.updated, .welcome-panel,
				.installer-q-icon {
					display: none;
				}
			</style>
