...
class WR_Nitro_Pluggable_WooCommerce
{
    ...
    public static function initialize()
    {
        ...
        add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_template_single_rating'], 9);
        add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_template_single_price'], 11);
        add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_template_single_share'], 50);
        add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_message_sale'], 11);

        add_action('woocommerce_after_single_product_summary', [__CLASS__, 'nitro_before_upsell_display'], 14);
        add_action('woocommerce_after_single_product_summary', [__CLASS__, 'woocommerce_output_recent_viewed_products'], 21);
        add_action('woocommerce_after_single_product_summary', [__CLASS__, 'nitro_after_related_products'], 22);

        // Support WooCommerce Simple Auction plugin
        add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_simple_aution_plugin'], 15);

        // Change product rating position.
        add_action('woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_rating', 15);

        // Add sidebar to shop page
        add_action('woocommerce_before_product_list', [__CLASS__, 'update_before_archive_product']);
        add_action('woocommerce_after_shop_loop', [__CLASS__, 'update_after_archive_product']);
        add_action('wr_nitro_before_header', [__CLASS__, 'woocommerce_mobile_sidebar']);

        // Add sidebar to product details
        add_action('woocommerce_before_single_product', [__CLASS__, 'update_before_single_product']);
        add_action('woocommerce_after_single_product', [__CLASS__, 'update_after_single_product']);

        // Add sidebar to cart page
        add_action('woocommerce_before_cart', [__CLASS__, 'update_before_cart_page']);
        add_action('woocommerce_after_cart', [__CLASS__, 'update_after_cart_page']);

        // Add sidebar to checkout page
        add_action('woocommerce_after_checkout_form', [__CLASS__, 'update_after_checkout_page']);

        // Change layout of single product
        add_action('template_redirect', [__CLASS__, 'nitro_template_single']);

        // Update notice success when reset password
        add_action('woocommerce_reset_password_notification', [__CLASS__, 'setcookie_first_reset_pass']);
        add_action('wp_loaded', [__CLASS__, 'show_notice_reset_pass'], 30);

        // Register additional sidebar for WooCommerce.
        add_action('widgets_init', [__CLASS__, 'widgets_init']);

        // Customize product quick view.
        add_action('wp_ajax_wr_quickview', [__CLASS__, 'wr_quickview']);
        add_action('wp_ajax_nopriv_wr_quickview', [__CLASS__, 'wr_quickview']);
        add_action('wp_ajax_wr_quickbuy', [__CLASS__, 'wr_quickbuy']);
        add_action('wp_ajax_nopriv_wr_quickbuy', [__CLASS__, 'wr_quickbuy']);

        // Delete product in wishlish
        add_action('wp_ajax_wr_remove_product_wishlish', [__CLASS__, 'remove_product_wishlish']);
        add_action('wp_ajax_nopriv_wr_remove_product_wishlish', [__CLASS__, 'remove_product_wishlish']);

        // Delete product in cart
        add_action('wp_ajax_wr_product_remove', [__CLASS__, 'product_remove']);
        add_action('wp_ajax_nopriv_wr_product_remove', [__CLASS__, 'product_remove']);

        // Get add to cart message
        add_action('wp_ajax_wr_add_to_cart_message', [__CLASS__, 'add_to_cart_message']);
        add_action('wp_ajax_nopriv_wr_add_to_cart_message', [__CLASS__, 'add_to_cart_message']);

        // Get add to cart error
        add_action('wp_ajax_wr_add_to_cart_error', [__CLASS__, 'add_to_cart_error']);
        add_action('wp_ajax_nopriv_wr_add_to_cart_error', [__CLASS__, 'add_to_cart_error']);

        // Register Ajax action to clear cart.
        add_action('wp_ajax_wr_clear_cart', [__CLASS__, 'clear_cart']);
        add_action('wp_ajax_nopriv_wr_clear_cart', [__CLASS__, 'clear_cart']);

        // Change button class of variable product
        add_action('woocommerce_single_variation', [__CLASS__, 'change_single_variation_add_to_cart_button'], 20);
        ...
        add_action('woocommerce_product_options_pricing', [__CLASS__, 'add_custom_general_fields']);
        add_action('woocommerce_product_options_downloads', [__CLASS__, 'add_custom_general_fields_options_downloadable']);
        add_action('woocommerce_process_product_meta', [__CLASS__, 'add_custom_general_fields_save']);

        // General init action
        add_action('init', [__CLASS__, 'general_wc_init']);

        add_action('template_redirect', [__CLASS__, 'add_product_viewed']);

        // Add icon switch layout
        add_action('woocommerce_before_shop_loop', [__CLASS__, 'switch_layout'], 9999);

        // Remove WooCommerce default styles.
        add_filter('woocommerce_enqueue_styles', '__return_false');

        // Customize search form.
        add_filter('get_product_search_form', [__CLASS__, 'get_product_search_form']);

        // Customize product tabs.
        add_filter('woocommerce_product_tabs', [__CLASS__, 'woocommerce_product_tabs'], 100);

        // Change number of products displayed per page
        add_filter('loop_shop_per_page', [__CLASS__, 'change_product_per_page']);

        // Switch layout grid and list in product list.
        add_filter('wr_nitro_options', [__CLASS__, 'switch_layout_products'], 20);

        // Add product title when added to wishlist
        add_filter('yith_wcwl_product_added_to_wishlist_message', [__CLASS__, 'add_title_to_wishlist']);

        // Add form fields Image title for product category
        add_action('product_cat_add_form_fields', [__CLASS__, 'add_category_fields'], 20);
        add_action('product_cat_edit_form_fields', [__CLASS__, 'edit_category_fields'], 20);

        add_action('created_term', [__CLASS__, 'save_category_fields'], 20, 3);
        add_action('edit_term', [__CLASS__, 'save_category_fields'], 20, 3);

        add_filter('woocommerce_available_variation', [__CLASS__, 'woocommerce_available_variation'], 20, 3);

        add_filter('woocommerce_cart_redirect_after_error', [__CLASS__, 'woocommerce_cart_redirect_after_error']);

        // Change image size of product category
        add_filter('subcategory_archive_thumbnail_size', [__CLASS__, 'wr_adjust_product_cat_thumbnail_size']);

        // Remove some settings unused of plugin YITH Compare premium
        if (function_exists('yith_woocompare_premium_constructor')) {
            add_filter('yith_woocompare_general_settings', [__CLASS__, 'remove_settings_yith_compare_premium'], 10, 1);
        }

        // Add param total price
        add_filter('woocommerce_add_to_cart_fragments', [__CLASS__, 'total_price_cart']);

        // Overwrite filter of Woocommerce Gift Card plugin
        if (is_plugin_active('woocommerce-giftcard/woocommerce-giftcard.php')) {
            add_filter('woocommerce_loop_add_to_cart_link', [__CLASS__, 'woocommerce_loop_add_to_cart_link'], 20, 2);
        }
        ...
        add_action('get_header', [__CLASS__, 'action_cart_ajax'], 999999999);

        // Custom flexslider of WC
        add_filter('woocommerce_single_product_carousel_options', [__CLASS__, 'custom_flexslider']);

        // Handle cart updated action to support 'Buy Now' feature.
        add_action('woocommerce_cart_updated', [__CLASS__, 'handle_cart_updated']);

        // Change number of related products on product page
        add_filter('woocommerce_output_related_products_args', [__CLASS__, 'wr_related_products_args']);

        // Add filter to prepare data for WooCommerce Products Filter.
        add_filter('woof_get_request_data', [__CLASS__, 'woof_get_request_data']);
    ...
    public static function nitro_template_single()
    {
        ...
        WR_Nitro_Helper::remove_action('woocommerce_single_product_summary', ['YITH_Woocompare_Frontend', 'add_compare_link'], 35);
    }
    ...
    public static function woocommerce_loop_add_to_cart_link($add_to_cart_html, $product)
    {
        ...
        if (!$is_giftcard) {
            ...
            return sprintf(
                '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="product__btn_cart product__btn ' . $class_icon,
    ...
    public static function switch_layout_products($wr_nitro_options)
    {
        ...
        if (isset($_GET['switch']) && in_array($_GET['switch'], ['grid', 'masonry', 'list'])) {
    ...
    public static function general_wc_init()
    {
        ...
        if (isset($_GET['wr-buy-now'])) {
            // Remove wr page loader html
            add_filter('wr_page_loader', [__CLASS__, 'empty_callback']);

            // Remove custom styles
            add_filter('wr_custom_styles', [__CLASS__, 'wr_custom_styles']);

            // Remove custom styles
            add_filter('wr_header_custom_css', [__CLASS__, 'empty_callback']);

            // Add value buy now to url Thank you page
            add_filter('woocommerce_get_checkout_order_received_url', [__CLASS__, 'checkout_order_received_url'], 10, 1);
        }
        ...
        if ($single_style !== '5') {
            switch ($position) {
                case 'after_title':
                    ...
                    add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_render_custom_content'], 5);

                    break;

                case 'after_price':

                    add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_render_custom_content'], 12);

                    break;

                case 'after_except':

                    add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_render_custom_content'], 20);
                ...
                case 'after_add_cart':
                    ...
                    if ($single_style != '1') {
                        add_action('woocommerce_single_product_summary', [__CLASS__, 'nitro_render_custom_content'], 32);
                    }
    ...
    public static function add_to_cart_message()
    {
        ...
        $data = ['message' => apply_filters('wc_add_to_cart_message', $message, $product_id)];
    ...
    public static function wr_quickbuy()
    {
        ...
        if ($wc_buynow_btn == 1) {
            ...
            } elseif ($wc_buynow_payment_info == 2) {
                $data['type'] = 'redirect';
            }
    ...
    public static function woocommerce_share()
    {
        ...
        // Get theme options
        $wr_nitro_options = WR_Nitro::get_options();
        if (!$wr_nitro_options['wc_single_social_share']) {
            return;
        }
    ...
    public static function change_single_variation_add_to_cart_button()
    {
        ...
        if ($check_gravityforms || (get_option('woocommerce_enable_ajax_add_to_cart_single') == 'no' && !(int) $wr_nitro_options['wc_buynow_btn'])) {
			...
			<?php if (wp_is_mobile() && $wr_nitro_options['wc_detail_mobile_sticky_cart']) {
            echo '<div class="p-action-sticky body_bg fc jcc">';
        } ?>
        ...
        woocommerce_quantity_input(['input_value' => isset($_POST['quantity']) ? wc_stock_amount($_POST['quantity']) : 1]);
    ...
    public static function customize_checkout_template()
    {
        ...
        wc_get_template('checkout/checkout-simple.php', ['checkout' => $checkout]);
    }
