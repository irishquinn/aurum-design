...
class WR_Nitro_Pluggable
{
    ...
    public static function initialize()
    {
        ...
        add_action('tgmpa_register', [__CLASS__, 'tgmpa_register']);
        add_action('init', [__CLASS__, 'widgets_init']);
        add_action('template_redirect', [__CLASS__, 'under_construction_page']);
        add_action('wr_activate', [__CLASS__, 'active_required_plugin']);
        add_action('init', [__CLASS__, 'plugins_wp_super_cache']);

        // Register Ajax action to backup / restore settings.
        add_action('wp_ajax_nitro_backup_settings', [__CLASS__, 'backup_settings']);
        add_action('wp_ajax_nitro_restore_settings', [__CLASS__, 'restore_settings']);

        // Register filter to prepare theme option values.
        add_filter('option_theme_mods_' . get_stylesheet(), [__CLASS__, 'prepare_settings']);
        add_filter('pre_update_option_theme_mods_' . get_stylesheet(), [__CLASS__, 'prepare_settings']);

        // Register Ajax action to install / uninstall plugin.
        add_action('wp_ajax_nitro_install_plugin', [__CLASS__, 'install_plugin']);
        add_action('wp_ajax_nopriv_nitro_install_plugin', [__CLASS__, 'install_plugin']);
        add_action('wp_ajax_nitro_uninstall_plugin', [__CLASS__, 'uninstall_plugin']);
        ...
        add_action('customize_register', ['WR_Nitro_Customize', 'initialize']);
        add_action('customize_controls_enqueue_scripts', ['WR_Nitro_Customize', 'customize_assets']);
        add_action('customize_controls_print_scripts', ['WR_Nitro_Customize', 'create_loading_mask']);
        add_action('customize_preview_init', ['WR_Nitro_Customize', 'customize_preview_assets']);
        add_action('customize_save_after', ['WR_Nitro_Customize', 'post_save_theme_mods']);

        // Setup actions / filters to render theme.
        add_action('wp_enqueue_scripts', ['WR_Nitro_Render', 'enqueue_scripts'], 10000); // Priority more than Nitro_Toolkit_Shortcode::enqueue_scripts
        add_action('wp_enqueue_scripts', ['WR_Nitro_Render', 'custom_styles'], 10001);
        add_action('wp_footer', ['WR_Nitro_Render', 'custom_inline_js']);
        ...
        add_filter('body_class', ['WR_Nitro_Render', 'body_class']);

        // Add setting enable AJAX add to cart buttons on single product
        add_filter('woocommerce_product_settings', [__CLASS__, 'setting_ajax_single_product']);
        ...
        if (is_admin()) {
            ...
            add_action('wp_ajax_nitro_enable_collector', [__CLASS__, 'enable_data_collector']);
        } else {
            // Add admin header nodes.
            add_action('wp_before_admin_bar_render', ['WR_Nitro_Header_Builder', 'edit_header_in_bar']);
        }
    ...
    protected static function collect_usage_data()
    {
        if (1 === ( int ) get_option('nitro_usage_data_collector')) {
            ...
            wp_remote_post('http://demo.woorockets.com/ittools/collect', [
                'body' => [
                    'siteurl' => site_url(),
