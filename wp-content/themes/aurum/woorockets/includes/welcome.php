...
class WR_Nitro_Welcome
{
    ...
    public static function initialize()
    {
        ...
        add_action('wp_ajax_nitro_refresh_nonce', [__CLASS__, 'refresh_nonce']);
        add_action('wp_ajax_nopriv_nitro_refresh_nonce', [__CLASS__, 'refresh_nonce']);
    }
    ...
    public static function enqueue_scripts()
    {
        ...
        if ($pagenow == 'admin.php' && isset($_GET['page']) && $_GET['page'] == 'wr-intro') {
            ...
            wp_enqueue_script('magnific-popup', get_template_directory_uri() . '/assets/3rd-party/magnific-popup/jquery-magnific-popup.min.js', [], false, true);
            wp_enqueue_style('wr-nitro-admin', get_template_directory_uri() . '/assets/woorockets/css/admin/admin.css');
            wp_enqueue_script('wr-nitro-admin', get_template_directory_uri() . '/assets/woorockets/js/admin/admin.js', ['jquery'], false, true);

            // Localize text translation.
            wp_localize_script('wr-nitro-admin', 'wr_nitro_admin', [
    ...
    protected static function demos_html($registered)
    {
        ...
        if (@count($packages)) :
            ...
            $package['tags'] = isset($package['tags']) ? ( array ) $package['tags'] : [];
						...
						<div>
							<?php if (isset($package['id'])) : ?>
								<a href="#TB_inline?inlineId=" class="button button-primary install-sample<?php
                                    ...
                                    echo esc_attr($package['id']); ?>" title="<?php
                                    esc_attr_e('Install sample data', 'wr-nitro'); ?>" data-width="50%" data-height="345"<?php
                                    if ($registered) {
								...
								<a href="#TB_inline?inlineId=" class="button button-primary uninstall-sample <?php
                                    ...
                                    echo esc_attr($package['id']); ?>" title="<?php
                                    esc_attr_e('Uninstall sample data', 'wr-nitro'); ?>" target="thickbox" data-width="50%" data-height="265">
        ...
        endforeach; else :
			...
			<div class="error"><p><?php
                _e('Failed to get available sample data packages.', 'wr-nitro'); ?></p></div>
			<?php
            endif;
    ...
    protected static function plugins_html($registered)
    {
		...
		<div id="plugins" class="tab-pane" role="tabpanel">
			...
			<div class="box-wrap plugins three-col">
				...
				<div class="col">
					<div class="box <?php if (isset($plugin['required']) && $plugin['required']) {
                        echo 'badge required';
                    } ?>">
						<?php if (isset($plugin['thumb']) && !empty($plugin['thumb'])) : ?>
							<?php if (isset($plugin['link']) && !empty($plugin['link'])) : echo '<a target="_blank" rel="noopener noreferrer" href="' . esc_url($plugin['link']) . '">';
        endif; ?>
								<img src="<?php echo esc_url($plugin['thumb']); ?>">
							<?php if (isset($plugin['link']) && !empty($plugin['link'])) : echo '</a>';
        endif; ?>
						<?php endif; ?>
						<div class="box-info">
							<h5>
			...
			<div class="welcome-panel panel-small envato-validate">
				...
				<form id="wr_registration_form" method="post" action="options.php">
					...
					<input class="required-field" id="nitro_customer_name" type="text" name="nitro_customer[name]" <?php //echo ( $nitro_customer['purchase_code'] ? 'disabled' : '' );?> value="<?php
                        echo esc_attr($nitro_customer['name']); ?>" placeholder="<?php
					...
					<input class="required-field" id="nitro_customer_email" type="text" name="nitro_customer[email]" <?php //echo ( $nitro_customer['purchase_code'] ? 'disabled' : '' );?> value="<?php
                        echo esc_attr($nitro_customer['email']); ?>" placeholder="<?php
					...
					<input class="required-field last" id="nitro_customer_purchase_code" type="text" name="nitro_customer[purchase_code]" <?php //echo ( $nitro_customer['purchase_code'] ? 'disabled' : '' );?> value="<?php
                        echo esc_attr($nitro_customer['purchase_code']); ?>" placeholder="<?php
