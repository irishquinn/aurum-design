...
class WR_Nitro_Customize_Control_Preset extends WP_Customize_Control
{
    ...
    public function enqueue()
    {
        ...
        if (!isset($enqueued)) {
            wp_enqueue_script('wr-nitro-customize-preset', get_template_directory_uri() . '/assets/woorockets/js/admin/customize/control/preset.js', [], '1.0.0', true);

            wp_localize_script('wr-nitro-customize-preset', 'wr_nitro_customize_preset', [
                'type' => $this->type,
    ...
    public function render_content()
    {
		...
		<div class="customize-control-content" id="wr-<?php echo esc_attr($this->type); ?> - <?php echo esc_attr($this->id); ?>">
			...
			<ul class="wr-select-image-container">
				...
				<li class="wr-select-image <?php echo esc_attr($val); if (checked($value, $val, false)) {
                    echo ' selected';
                } ?>" data-title="<?php echo esc_attr($label['title']); ?>">
					<ul class="colors-preset">
