...
class WR_Nitro_Meta_Box extends RW_Meta_Box
{
    ...
    public static function initialize()
    {
        ...
        add_action('admin_init', [__CLASS__, 'register_meta_boxes']);

        // Register necessary actions / filters to hook WR Nitro meta boxes into WordPress.
        add_action('admin_enqueue_scripts', [__CLASS__, 'enqueue_admin_assets']);

        add_filter('rwmb_meta_boxes', [__CLASS__, 'meta_boxes']);
        add_filter('rwmb_outside_conditions', [__CLASS__, 'outside_conditions']);

        // Register filter to verify page option values before saving.
        add_filter('update_post_metadata', [__CLASS__, 'verify'], 10, 5);

        // Register filter to synchronize default option values with current theme customizer values.
        add_filter('rwmb_field_meta', [__CLASS__, 'get_meta'], 10, 3);

        // Register filter to allow saving empty post.
        add_filter('wp_insert_post_empty_content', [__CLASS__, 'allow_saving_empty_post'], 10, 2);
    ...
    public static function enqueue_admin_assets()
    {
        ...
        $types = ['post', 'page', 'nitro-gallery', 'product'];
        ...
        if (isset($post->post_type) && in_array($post->post_type, ['product'])) {
            wp_enqueue_script('wr-metabox', get_template_directory_uri() . '/assets/woorockets/js/admin/meta-box/metabox.js', [], false, true);
        }
    }
        ...
        $meta_boxes[] = [
            ...
            'fields' => [
                ...
                [
                    ...
                    'std' => 1170,
                    'hidden' => ['wr_layout_content_width_unit', '!=', 'pixel'],
                    'tab' => esc_html__('General', 'wr-nitro'),
                    'tab_id' => 'general',
                ],
                [
                    ...
                    'std' => 100,
                    'hidden' => ['wr_layout_content_width_unit', '!=', 'percentage'],
                    'tab' => esc_html__('General', 'wr-nitro'),
                    'tab_id' => 'general',
                ],
                [
                ...
                [
                    ...
                    'std' => 'primary-sidebar',
                    'hidden' => ['wr_page_layout', '=', 'no-sidebar'],
                    'tab' => esc_html__('Page Content', 'wr-nitro'),
                    'tab_id' => 'page-content',
                ],
                [
                ...
                [
                    ...
                    'visible' => ['wr_page_layout_bg_image', '>', 0],
                    'tab' => esc_html__('Page Content', 'wr-nitro'),
                    'tab_id' => 'page-content',
                ],
                [
        ...
        $meta_boxes[] = [
            ...
            'fields' => [
                ...
                [
                    ...
                    'max_file_uploads' => 1,
                    'mime_type' => 'video',
                    'visible' => ['wc_product_video', '=', 'file'],
                ],
                [
                    'name' => esc_html__('Video Link', 'wr-nitro'),
                    'id' => 'wc_product_video_url',
                    'type' => 'oembed',
                    'visible' => ['wc_product_video', '=', 'url'],
                ],
            ]
        ];
    ...
    public static function outside_conditions()
    {
        ...
        if (isset($post->post_type) && in_array($post->post_type, $types)) {
            $conditions['.wr-tab-page-title, .wr-tab-page-content'] = [
                'hidden' => ['global_opt', 1]
            ];
            return $conditions;
        }
    ...
    public function show()
    {
        ...
        foreach ($this->fields as $field) {
            ...
            if (method_exists(__CLASS__, 'get_class_name')) {
                call_user_func([self::get_class_name($field), 'show'], $field, $saved);
