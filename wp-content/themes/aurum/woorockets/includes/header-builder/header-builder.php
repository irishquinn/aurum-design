...
class WR_Nitro_Header_Builder
{
    ...
    public static function initialize()
    {
        ...
        add_action('admin_init', [__CLASS__, 'enqueue_admin_assets'], 9999);
        add_action('wr_activate', [__CLASS__, 'creat_header_default']);

        // Register Ajax actions for building and managing header.
        add_action('wp_ajax_set_to_default', [__CLASS__, 'set_to_default_in_header']);
        add_action('wp_ajax_get_menu_html', [__CLASS__, 'get_menu_html']);
        add_action('wp_ajax_header_builder', [__CLASS__, 'get_data']);
        add_action('wp_ajax_hb_get_data', [__CLASS__, 'get_header_data']);
        add_action('wp_ajax_duplicate', [__CLASS__, 'duplicate']);
        add_action('wp_ajax_save_data', [__CLASS__, 'save_data']);
        add_action('wp_ajax_save_file', [__CLASS__, 'save_file']);

        // Refine setting in row action
        add_filter('post_row_actions', [__CLASS__, 'row_actions'], 2, 100);

        // Set default in customize
        add_action('customize_save_after', [__CLASS__, 'set_to_default_in_customize']);

        // Refine column in list header page
        add_filter('manage_header_builder_posts_columns', [__CLASS__, 'columns_filter'], 10, 1);
        add_action('manage_header_builder_posts_custom_column', [__CLASS__, 'render_columns'], 10, 2);

        add_filter('post_updated_messages', [__CLASS__, 'updated_messages']);
        add_filter('bulk_post_updated_messages', [__CLASS__, 'bulk_updated_messages']);
        ...
        if (($pagenow == 'post-new.php' || $pagenow == 'post.php') && $post_type_current == 'header_builder') {
            ...
            add_filter('vc_check_post_type_validation', [__CLASS__, 'post_type_validation']);

            add_action('edit_form_after_title', [__CLASS__, 'render_html'], 999999);
            ...
            add_action('save_post_header_builder', [__CLASS__, 'save_post'], 10, 1);

            add_filter('tiny_mce_before_init', [__CLASS__, 'add_blur_change_tiny_mce'], 9999, 1);
        }
        ...
        if (is_plugin_active('amazon-s3-and-cloudfront/wordpress-s3.php') || is_plugin_active('amazon-s3-and-cloudfront-pro/amazon-s3-and-cloudfront-pro.php')) {
            add_action('init', [__CLASS__, 'remove_filter_as3cf'], 20);
        }

        add_action('wp_insert_post', [__CLASS__, 'convert_serialize'], 9999, 2);
        ...
        add_filter('wpml_copy_from_original_custom_fields', [__CLASS__, 'wpml_copy_from_original_custom_fields']);
    ...
    public static function remove_filter_as3cf()
    {
        ...
        if ($post_type_current == 'header_builder' || ($pagenow == 'post.php' && isset($_REQUEST['post_type']) && $_REQUEST['post_type'] == 'header_builder')) {
            WR_Nitro_Helper::remove_action('the_content', ['AS3CF_Local_To_S3', 'filter_post'], 100);
            WR_Nitro_Helper::remove_action('the_excerpt', ['AS3CF_Local_To_S3', 'filter_post'], 100);
            WR_Nitro_Helper::remove_action('content_edit_pre', ['AS3CF_Local_To_S3', 'filter_post'], 10);
            WR_Nitro_Helper::remove_action('excerpt_edit_pre', ['AS3CF_Local_To_S3', 'filter_post'], 10);
            WR_Nitro_Helper::remove_action('content_save_pre', ['AS3CF_S3_To_Local', 'filter_post'], 10);
            WR_Nitro_Helper::remove_action('excerpt_save_pre', ['AS3CF_S3_To_Local', 'filter_post'], 10);
        }
    }
    ...
    {
        $list_layout = ['horizontal', 'vertical'];
    ...
    public static function enqueue_admin_assets()
    {
        ...
        if ($post_type_current == 'header_builder') {
            ...
            if ($wr_is_wpml_activated && ($pagenow == 'post-new.php' || $pagenow == 'post.php')) {
                wp_enqueue_style('wr-header-builder-wpml', ICL_PLUGIN_URL . '/res/css/language-selector.css', [], ICL_SITEPRESS_VERSION);
            }
            ...
            wp_enqueue_script('wr-header-builder-admin', $uri_directory . '/assets/woorockets/js/admin/header-builder/admin.js', [], false, true);
            ...
            if ($pagenow == 'post-new.php' || $pagenow == 'post.php') {
                ...
                // Enqueue jquery-ui.
                wp_enqueue_script('jquery-ui', $uri_directory . '/assets/3rd-party/jquery-ui/jquery-ui.min.js', [], false, true);

                // Enqueue spectrum color.
                wp_enqueue_script('spectrum-color', $uri_directory . '/assets/3rd-party/spectrum/spectrum.js', [], false, true);

                // Enqueue HB libs.
                wp_enqueue_script('wr-header-builder-libs', $uri_directory . '/assets/woorockets/js/admin/header-builder/libs.js', [], false, true);

                // Enqueue sortable js.
                wp_enqueue_script('sortable-js', $uri_directory . '/assets/3rd-party/sortable-js/sortable.min.js', [], false, true);

                // Enqueue Backbone Model.
                wp_enqueue_script('backbone-model', $uri_directory . '/assets/3rd-party/backbone/backbone-model.js', [], false, true);

                // Enqueue Backbone View.
                wp_enqueue_script('backbone-view', $uri_directory . '/assets/3rd-party/backbone/backbone-view.js', [], false, true);

                // Enqueue HB Models.
                wp_enqueue_script('wr-header-builder-model', $uri_directory . '/assets/woorockets/js/admin/header-builder/models.js', [], false, true);

                // Enqueue HB Models.
                wp_enqueue_script('wr-header-builder-view', $uri_directory . '/assets/woorockets/js/admin/header-builder/views.js', [], false, true);
    ...
    public static function get_menu_html()
    {
        ...
        if (isset($_POST['menu_id'])) {
            $id = $_POST['menu_id'];
            wp_nav_menu(['menu' => $id, 'depth' => 1, 'container' => '', 'fallback_cb' => null]);
        }
        exit;
    }
    ...
    public static function get_data()
    {
        ...
        if (isset($_REQUEST['menu_id'])) {
            wp_nav_menu([menu => $_REQUEST['menu_id'], depth => 1]);
        }
    ...
    public static function fillter_property($list_property)
    {
        if (!(isset($list_property['backgroundImage']) && $list_property['backgroundImage'])) {
            unset($list_property['backgroundImage'] , $list_property['backgroundPosition'] , $list_property['backgroundRepeat'] , $list_property['backgroundSize']);
        }

        if (!(isset($list_property['borderStyle']) && $list_property['borderStyle'] != 'none')) {
            unset($list_property['borderStyle'] , $list_property['borderBottomWidth'] , $list_property['borderColor'] , $list_property['borderLeftWidth'] , $list_property['borderRightWidth'] , $list_property['borderTopWidth']);
        }
     ...
     * @return  array
     */
    public static function array_fillter_recursive($array_fillter, $array_data, $theme_option = [], $key_el = null, $use_default = false)
    {
        if ($array_data) {
    ...
    public static function data_color_theme($key_item, $theme_option)
    {
        ...
        $data = isset($data[$key_item]) ? $data[$key_item] : [];
