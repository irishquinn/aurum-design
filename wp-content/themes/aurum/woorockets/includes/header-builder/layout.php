...
$wr_list_layout = ['horizontal', 'vertical'];
...
if ($wr_custom_post_types) {
    foreach ($wr_custom_post_types as $key => $val) {
        if (in_array($key, ['revision', 'attachment', 'nav_menu_item', 'page'])) {
            continue;
        }
...
========================================================== ***/
$wr_custom_post_type_archives = [];
$wr_custom_post_type_archives_slug = [];

if ($wr_custom_post_types) {
    foreach ($wr_custom_post_types as $key => $val) {
        if (in_array($key, ['revision', 'attachment', 'nav_menu_item']) || !$val->has_archive) {
            continue;
        }
...
if ($wr_custom_post_types) {
    foreach ($wr_custom_post_types as $key => $val) {
        if (in_array($key, ['revision', 'attachment', 'nav_menu_item'])) {
            continue;
        }
        ...
        if ($wr_post_taxes) {
            foreach ($wr_post_taxes as $key_item => $val_item) {
                if ($key == 'post') {
                    if (!in_array($key_item, ['category', 'post_tag'])) {
                        continue;
                    }
                } elseif ($key == 'product') {
                    if (!in_array($key_item, ['product_cat', 'product_tag'])) {
                        continue;
                    }
                }
...
if ($wr_show_on_front == 'posts') {
    ...
    unset($wr_hide_show_fixed['pages']['data'][$wr_page_on_front] , $wr_hide_show_fixed['pages']['data'][$wr_page_for_posts] , $wr_hide_show_fixed['pages']['data']['blog']);
...
<div id="hb-app" data-id="<?php echo intval($wr_nitro_header_id); ?>" class="hb-wrapper <?php echo 'hb-' . $wr_hb_layout ?>" style="display:none ">
	...
	<div class="hb-wrapper-action">
		...
		<div id="load-template">
			...
			<p class="des-import"><?php echo wp_kses(__('or you can <span>import json backup file from your computer</span>.', 'wr-nitro'), ['span' => []]); ?></p>
		</div>
							...
							<div class="content-group">
								<div class="list-chb">
									<div class="list-chb social">
										<?php
                                        $wr_channels = ['facebook', 'twitter', 'instagram', 'linkedin', 'pinterest', 'dribbble', 'behance', 'flickr', 'google-plus', 'medium', 'skype',  'slack', 'tumblr', 'vimeo', 'yahoo', 'youtube', 'rss', 'vk'];
                                        $wr_list_social = [];
					...
					<div class="list">
						<?php
                            ...
                            for ($i = 1; $i <= $wr_number; $i++) {
                                ?>
								<div class="item">
									<h3 class="name-header"><?php echo esc_attr($wr_title); ?> - <?php echo absint($i); ?> <?php if ($wr_hb_layout == 'horizontal' && ($i == 3 || $i == 6)) {
                                    echo ' (' . __('Background transparent', 'wr-nitro') . ')';
                                } ?></h3>
									<div class="img">
