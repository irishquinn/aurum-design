...
class WR_Nitro_Assets
{
     ...
     * @var  array
     */
    protected static $lazy_load_sources = [];
     ...
     * @var  array
     */
    protected static $compressed_handles = [];
    ...
    public static function admin_init()
    {
        ...
        if (self::COMPRESS_CUSTOMIZE_ASSETS && 'customize.php' == $pagenow) {
            add_action('customize_controls_enqueue_scripts', [__CLASS__, 'compress_customize_assets'], 9999999999);

            // Register actions to print inline scripts.
            add_action('customize_controls_print_footer_scripts', [__CLASS__, 'print_inline_scripts'], 9999999999);
        } elseif (self::COMPRESS_PAGE_OPTIONS_ASSETS && in_array($pagenow, ['post-new.php', 'post.php'])) {
            add_action('admin_enqueue_scripts', [__CLASS__, 'compress_page_assets'], 9999999999);
            add_action('admin_footer', [__CLASS__, 'compress_page_assets'], 9999999999);

            // Register actions to print inline scripts.
            add_action('admin_print_footer_scripts', [__CLASS__, 'print_inline_scripts'], 9999999999);
        }
    }
    ...
    public static function init()
    {
        ...
        add_action('wp_enqueue_scripts', [__CLASS__, 'optimize_site_assets'], 9999999997);
        ...
        if (( int ) $wr_nitro_options['compress_css']) {
            ...
            add_action('wp_enqueue_scripts', [__CLASS__, 'compress_site_styles'], 9999999998);
        }

        if (( int ) $wr_nitro_options['compress_js']) {
            // Register actions to compress and combine scripts.
            add_action('wp_enqueue_scripts', [__CLASS__, 'compress_site_scripts'], 9999999999);

            // Register actions to print inline scripts.
            add_action('wp_print_footer_scripts', [__CLASS__, 'print_inline_scripts'], 9999999999);
        }
    }
    ...
    protected static function compress_admin_styles($screen)
    {
        ...
        foreach ($wp_styles->to_do as $handle) {
            if (isset($wp_styles->registered[$handle]->_compressed) && $wp_styles->registered[$handle]->_compressed) {
                ...
                $wp_styles->registered[$handle]->deps = [];
            }
        }
    ...
    public static function compress_site_styles()
    {
        ...
        // Get all stylesheets including dependencies.
        $styles = $queue = $inline = $last_modification = [];
        $index = wp_generate_password(8, false);
        ...
        foreach ($styles as $handle => $sources) {
            ...
            $wp_styles->add($handling, $compression, [], false, $media);
    ...
    public static function compress_site_scripts()
    {
        ...
        // Get all scripts including dependencies.
        $scripts = $inline = $queue = $last_modification = [];
        $index = wp_generate_password(8, false);
        ...
        foreach ($scripts as $handle => $sources) {
            ...
            $wp_scripts->add($handling, $compression, [], false, null);
