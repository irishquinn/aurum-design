...
class WR_Nitro_Megamenu
{
    ...
    public static function initialize()
    {
        ...
        if (is_admin()) {
            // Save data megamenu
            add_action('wp_ajax_wr_save_megamenu', [__CLASS__, 'ajax_save_megamenu']);

            // Search product
            add_action('wp_ajax_wrmm_products', [__CLASS__, 'wrmm_products']);

            // Get list product
            add_action('wp_ajax_wrmm_get_products', [__CLASS__, 'wrmm_get_products']);
            ...
            if ($pagenow == 'nav-menus.php') {
                ...
                add_action('admin_footer', [__CLASS__, 'reset_data']);

                add_action('admin_footer', [__CLASS__, 'template_megamenu']);

                add_action('admin_footer', [__CLASS__, 'enqueue_admin_assets']);
            }
        } else {
            // Permission menu item
            add_filter('wp_get_nav_menu_items', [__CLASS__, 'permission_menu_item'], 10, 3);
    ...
    public static function permission_menu_item($items, $menu, $args)
    {
        ...
        foreach ($items as $key => $item) {
            ...
            $data_menu_item = isset(self::$data[$menu->term_id][$item->ID]) ? self::$data[$menu->term_id][$item->ID] : [];
    ...
    public static function get_term_product_category()
    {
        ...
        if ($list_category_children_along) {
            foreach ($list_category_children_along as $val) {
                $image = self::get_image_term_product_category($val->term_id, [100, 100]);

                $product_category[$val->term_id] = [
                    'name' => $val->name,
                    'count' => $val->count,
                    'image' => esc_url($image),
                    'level' => $val->level
...
<?php
    ...
    */
    public static function wrmm_get_products()
    {
        ...
        if (!isset($_POST['_nonce']) || !wp_verify_nonce($_POST['_nonce'], 'wr_theme_megamenu_nonce_check')) {
            exit(json_encode(['message' => 'The nonce check wrong.']));
        }

        // Check isset data
        if (!isset($_POST['list_id'])) {
            exit(json_encode(['message' => __('Data not isset.', 'wr-nitro')]));
        }

        $list_id = explode(',', $_POST['list_id']);

        $list_product = new WP_Query([
            'post_type' => 'product',
            'post__in' => $list_id,
            'orderby' => 'title',
            'order' => 'DESC',
        ...
        exit(json_encode(['message' => __('Product empty.', 'wr-nitro')]));
    ...
    */
    public static function wrmm_products()
    {
        ...
        add_filter('posts_search', [__CLASS__, 'search_where'], 10, 2);
        ...
        remove_filter('posts_search', [__CLASS__, 'search_where'], 10);
        ...
        exit(json_encode(['message' => __('No results.', 'wr-nitro')]));
    ...
    public static function enqueue_admin_assets()
    {
        ...
        wp_enqueue_script('spectrum-color', get_template_directory_uri() . '/assets/3rd-party/spectrum/spectrum.js', [], false, true);

        // Enqueue script for Mega Menu.
        wp_enqueue_script('wr-mega-menu', get_template_directory_uri() . '/assets/woorockets/js/admin/megamenu/megamenu.js', [], false, true);
    ...
    public static function ajax_save_megamenu()
    {
        // Check nonce
        if (!isset($_POST['_nonce']) || !wp_verify_nonce($_POST['_nonce'], 'wr_theme_megamenu_nonce_check')) {
            exit(json_encode(['status' => 'false', 'message' => __('The nonce check wrong.', 'wr-nitro')]));
        }

        // Check is menu
        if (!(isset($_POST['menu_id']) && ($_POST['menu_id'] == 0 || is_nav_menu($_POST['menu_id'])))) {
            exit(json_encode(['status' => 'false', 'message' => __('Menu ID is empty.', 'wr-nitro')]));
        }
        ...
        if ($data_post) {
            if (isset($_POST['data_last_update']) && $_POST['data_last_update'] == 'ok') {
                ...
                exit(json_encode(['status' => 'updating', 'list_id_updated' => $list_id_updated]));
            };
        }
    ...
    {
        // Get all menu
        $nav_menus = wp_get_nav_menus(['fields' => 'ids']);
        ...
        if ($nav_menus) {
            ...
            if ($data_mega) {
                ...
                foreach ($data_mega as $key => $val) {
                    ...
                    if ($val) {
                        // Get list menu item
                        $list_menu_item = wp_get_nav_menu_items($key, ['post_status' => 'any']);
    ...
    public static function localize_menus()
    {
        ...
        if ($data && isset($data[$menu]) && $data[$menu]) {
            ...
            foreach ($data[$menu] as $key => $val) {
                if (!(isset($val['element_type']) && isset($val['element_data']) && $val['element_data'])) {
                    continue;
                }
                ...
                if ($list_id) {
                        ...
                        foreach ($list_id as $key_item => $val_item) {
                            ...
                            if ($val_item > 0 && $product) {
                                if ($product->post->post_status == 'publish') {
                                    $data[$menu][$key]['element_data_product'][] = [
                                        'id' => $val_item,
                                        'title' => $product->get_title(),
                                        'image' => $product->get_image([50, 50]),
                                        'price' => $product->get_price_html(),
                    ...
                    } elseif ($val['element_type'] == 'element-categories') {
                        foreach ($list_id as $key_item => $val_item) {
                            $val_item = (int) $val_item;
                            $categories = get_term($val_item, 'product_cat', ARRAY_A);

                            if ($val_item > 0 && $categories) {
                                $image = self::get_image_term_product_category($val_item, [100, 100]);

                                $data[$menu][$key]['element_data_categories'][] = [
                                    'id' => $val_item,
    ...
    public static function frontend()
    {
        ...
        add_filter('header_builder_nav_menu_args', [__CLASS__, 'nav_menu_args']);
    }
