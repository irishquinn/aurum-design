...
class WR_Nitro_Sample_Data
{
    ...
    public static function initialize()
    {
        ...
        add_action('wp_ajax_nitro_install_sample_data', [__CLASS__, 'install_sample_data']);
        add_action('wp_ajax_nopriv_nitro_install_sample_data', [__CLASS__, 'install_sample_data']);
        add_action('wp_ajax_nitro_uninstall_sample_data', [__CLASS__, 'uninstall_sample_data']);

        // Register filter to alter URL for demo images.
        if ('remote' == self::$demo_images_storage && false !== get_option('wr_nitro_sample_package')) {
            add_filter('wp_get_attachment_url', [__CLASS__, 'get_attachment_url'], 10, 2);
            add_filter('wp_get_attachment_thumb_url', [__CLASS__, 'get_attachment_url'], 10, 2);
            add_filter('wp_get_attachment_image_src', [__CLASS__, 'get_attachment_image_src'], 10, 4);
            add_filter('wp_calculate_image_srcset', [__CLASS__, 'calculate_image_srcset'], 10, 5);
    ...
    protected static function import_full_demo_site($package, $xml, $theme, $child = null)
    {
        ...
        foreach ($xml->xpath('//product/extension') as $plugin) {
            ...
            if ($queries && count($queries)) {
                foreach ($queries as $query) {
                    ...
                    $attrs = isset($attrs['@attributes']) ? $attrs['@attributes'] : [];
        ...
        return isset($demo_assets) ? $demo_assets : [];
    }
