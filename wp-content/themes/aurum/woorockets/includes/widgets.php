...
class WR_Nitro_Widgets
{
    ...
    public static function initialize()
    {
        ...
        if (is_admin()) {
            ...
            add_action('in_widget_form', [__CLASS__, 'in_widget_form'], 10, 3);
            ...
            add_action('wp_ajax_widget-assignment-search-target', [__CLASS__, 'search_target']);
            add_action('wp_ajax_widget-assignment-load-target', [__CLASS__, 'load_target']);

            // Add filter to save widget assignment data.
            add_filter('widget_update_callback', [__CLASS__, 'update_widget'], 10, 3);
        }
        ...
        add_filter('widget_display_callback', [__CLASS__, 'prepare_widget']);
    }
    ...
    public static function in_widget_form($widget, $return, $instance)
    {
        ...
        $assignment = wp_parse_args(isset($instance['assignment']) ? $instance['assignment'] : '', [
            'audience' => 'all',
		...
		<div class="wr-nitro-widget-assignment">
			<p>
				...
				<select class="widefat wr-nitro-widget-assignment-audience" id="<?php
                    echo esc_attr($id_base . 'audience'); ?>" name="<?php
			...
			<p>
				...
				<select class="widefat wr-nitro-widget-assignment-visibility" id="<?php
                    echo esc_attr($id_base . 'visibility'); ?>" name="<?php
			...
			<p class="wr-nitro-widget-assignment-target-container loading">
				...
				<input type="hidden" class="widefat wr-nitro-widget-assignment-target" id="<?php
                    echo esc_attr($id_base . 'target'); ?>" name="<?php
                    echo esc_attr($name_base . '[target]'); ?>" value="<?php
    ...
    public static function search_target()
    {
        ...
        if (!empty($search) && strlen($search) >= 2) {
            ...
            foreach ($taxonomies as $taxonomy) {
                ...
                if (in_array($taxonomy, ['nav_menu', 'link_category', 'post_format'])) {
                    continue;
                }

                // Skip searching in WooCommerce's product types and shipping classes.
                elseif (in_array($taxonomy, ['product_type', 'product_shipping_class'])) {
                    continue;
                }
        ...
        wp_send_json_success(isset($return) ? $return : []);
    }
    ...
    public static function prepare_widget($instance)
    {
        ...
        $assignment = wp_parse_args($instance['assignment'], [
            'audience' => 'all',
            'visibility' => 'hidden on',
            'target' => ''
        ]);
