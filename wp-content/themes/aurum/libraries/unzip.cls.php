...
class UnZIP
{
     ...
     */
    public function __construct($p_source_zip_file = null, $p_target_location = null)
    {
    ...
    public function restrictedFileExtensions($p_file_extension_csv, $p_allow_file_without_extension = null)
    {
        ...
        $this->file_extension_restricted_flag = false;
        if (is_array($this->allowed_file_extensions) && count($this->allowed_file_extensions) > 0) {
            $this->file_extension_restricted_flag = true;
        }
        if (false == $this->allowed_file_without_extension) {
            $this->file_extension_restricted_flag = true;
        }
    ...
    {
        if (isset($p_source_zip_file)) {
            $this->setSource($p_source_zip_file);
        }
        if (isset($p_target_location)) {
            $this->setTarget($p_target_location);
        }
    }
     ...
     */
    public function setSourceZip($p_source_zip_file)
    {
        $this->setSource($p_source_zip_file);
    }
    ...
    public function setSource($p_source_zip_file)
    {
        if (!empty($p_source_zip_file)) {
            ...
            if (is_file($p_source_zip_file)) {
                $f_source = $this->source = $p_source_zip_file;
            } elseif (is_file($f_source)) {
                $this->source = $f_source;
            }
            if (!is_file($f_source)) {
                trigger_error('You must set a valid ZIP file as source [' . $f_source . ']');
            }
            $this->log[] = 'set source file - ' . $this->source;
        } else {
            trigger_error('You must set a valid ZIP file.[' . $p_source_zip_file . ']');
        }
    }
     ...
     */
    public function setTargetLocation($p_target_location)
    {
        $this->setTarget($p_target_location);
    }
    ...
    {
        if (empty($p_target_location)) {
            trigger_error('No target location set!');
        }
        if ($this->os != 'win' && substr($p_target_location, 0, 1) == '/') {
            $this->setTargetPathTypeAbsoulate();
        }
        if (!is_dir($p_target_location) && false == $this->target_path_type_absoulate) {
            if (!isset($this->source)) {
                trigger_error('Source ZIP didnot set yet. It may cause misbehave. I recommend you to set source ZIP first...', E_USER_WARNING);
            }
            ...
            if (isset($this->source)) {
                ...
                $rtloc = dirname(__FILE__) . DS;
            }
            $trgtdir = $rtloc . $f_target_location . DS;
        } else {
            $trgtdir = $p_target_location;
        }
        $this->target = $trgtdir;
    ...
    private function fixTarget()
    {
        if (isset($this->source)) {
            ...
            if (!isset($this->target)) {
                $this->_target = $source_pathinfo['dirname'] . DS;
            }
            $lstchr = substr($this->_target, strlen($this->_target) - 1);
            if ($lstchr != '/' && $lstchr != '\\') {
                $this->_target .= DS;
            }
            if (true == $this->Flag4CNFBE) {
                $this->_target .= $source_pathinfo['filename'] . DS;
            }
            $this->_target = str_replace(['/', '\\'], DS, $this->_target);
            ...
            if (!is_dir($this->_target)) {
                $this->mkdir($this->_target);
            }
            $this->log[] = 'target location fixed. extract location: ' . $this->_target;
        } else {
            trigger_error('You must set source ZIP file, otherwise target location proper determination not possible...');
        }
    }
    ...
    public function setFlag4CNFBE($p_fl_4CNFBE)
    {
        ...
        if (true == $this->Flag4CNFBE) {
            $this->log[] = 'create new folder with same name of UNZIP source file before extract and extract into that location.';
        } else {
            $this->log[] = 'just extract into target location. need not to create directory before extract...';
        }
    }
    ...
    {
        if (!isset($this->source) && !isset($p_source_zip_file)) {
            trigger_error('You must set source ZIP file...');
        }
        if (!isset($this->source) && isset($p_source_zip_file)) {
            $this->setSource($p_source_zip_file);
        }
            ...
            if ($zf = @zip_open($this->source)) {
                if (is_resource($zf)) {
                    ...
                    while ($entry = zip_read($zf)) {
                        ...
                        if (!$this->isDirectory($zp_entry, $zp_filesize)) {
                            $f_possible_files[] = ['entry' => $entry, 'target' => $this->_target . $zp_entry];
                        } else {
                            $f_possible_dirs[] = $zp_entry;
                        }
                    }
                    if (isset($f_possible_dirs) && is_array($f_possible_dirs)) {
                        $this->log[] = 'found ' . count($f_possible_dirs) . ' directory to create!';
                        foreach ($f_possible_dirs as $dr) {
                            $this->mkdir($this->_target . $dr);
                        }
                    }
                ...
                //echo '<pre>'.print_r($zp_entries, true).'</pre>';
                    ...
                    $this->log[] = 'error while open. no # ' . $zf . ' [ERROR Message: ' . $this->getErrorMessage($zf) . ']';
                }
            } else {
                trigger_error('Invalid ZIP format! Fail to read! SOURCE: ' . $this->source, E_USER_ERROR);
            }
        } else {
            trigger_error('Sorry! Source didnot set.', E_USER_ERROR);
        }
    ...
    private function isDirectory($p_path, $p_size = 0)
    {
        ...
        if (substr($p_path, strlen($p_path) - 1) != '/') {
            ...
            if (!isset($pthinfo['extension']) && $p_size == 0) {
                $rtrn = true;
            }
        }
        return $rtrn;
        ...
        if (is_resource($pi_zp_entry)) {
            ...
            if (true == $this->file_extension_restricted_flag) {
                ...
                if (isset($flparsed['extension']) && !empty($flparsed['extension'])) {
                    if (in_array($flparsed['extension'], $this->allowed_file_extensions)) {
                        $proceed2write = true;
                    } else {
                        $this->log[] = 'ignore for extension - ' . $flparsed['extension'];
                    }
                } elseif (true == $this->allowed_file_without_extension) {
                    $proceed2write = true;
                } else {
                    $this->log[] = 'no extension found. so ignored to write!';
                }
            }
            if (true == $proceed2write) {
                ...
                if ($fd = @fopen($f_fl, 'w+')) {
                    ...
                    $this->log[] = 'cannot open in write mode at target location. write abort!';
                }
            }
        } else {
            $this->log[] = 'supplied zip file entry is not valid resource. write abort!';
        }
    ...
    {
        $p_path = str_replace(['/', '\\'], DS, $p_path);
        if (!is_dir($p_path)) {
            $rtrn = @mkdir(addslashes($p_path), $this->chmod, true);
            if ($rtrn) {
                $this->log[] = 'directory created - ' . $p_path;
            } else {
                $this->log[] = 'sorry! fail to create directory - ' . $p_path;
            }
            return $rtrn;
        }
    }
    ...
    {
        return '<h3>LOG History:</h3><pre>' . print_r($this->log, true) . '</pre>';
    }

    private function getErrorMessage($p_error_no)
    {
        $errors = [
            1 => ['name' => 'ZipArchive::ER_MULTIDISK', 'message' => 'Multi-disk zip archives not supported.'],
            2 => ['name' => 'ZipArchive::ER_RENAME', 'message' => 'Renaming temporary file failed.'],
            3 => ['name' => 'ZipArchive::ER_CLOSE', 'message' => 'Closing zip archive failed'],
            4 => ['name' => 'ZipArchive::ER_SEEK', 'message' => 'Seek error'],
            5 => ['name' => 'ZipArchive::ER_READ', 'message' => 'Read error'],
            6 => ['name' => 'ZipArchive::ER_WRITE', 'message' => 'Write error'],
            7 => ['name' => 'ZipArchive::ER_CRC', 'message' => 'CRC error'],
            8 => ['name' => 'ZipArchive::ER_ZIPCLOSED', 'message' => 'Containing zip archive was closed'],
            9 => ['name' => 'ZipArchive::ER_NOENT', 'message' => 'No such file.'],
            10 => ['name' => 'ZipArchive::ER_EXISTS', 'message' => 'File already exists'],
            11 => ['name' => 'ZipArchive::ER_OPEN', 'message' => "Can't open file"],
            12 => ['name' => 'ZipArchive::ER_TMPOPEN', 'message' => 'Failure to create temporary file.'],
            13 => ['name' => 'ZipArchive::ER_ZLIB', 'message' => 'Zlib error'],
            14 => ['name' => 'ZipArchive::ER_MEMORY', 'message' => 'Memory allocation failure'],
            15 => ['name' => 'ZipArchive::ER_CHANGED', 'message' => 'Entry has been changed'],
            16 => ['name' => 'ZipArchive::ER_COMPNOTSUPP', 'message' => 'Compression method not supported.'],
            17 => ['name' => 'ZipArchive::ER_EOF', 'message' => 'Premature EOF'],
            18 => ['name' => 'ZipArchive::ER_INVAL', 'message' => 'Invalid argument'],
            19 => ['name' => 'ZipArchive::ER_NOZIP', 'message' => 'Not a zip archive'],
            20 => ['name' => 'ZipArchive::ER_INTERNAL', 'message' => 'Internal error'],
            21 => ['name' => 'ZipArchive::ER_INCONS', 'message' => 'Zip archive inconsistent'],
            22 => ['name' => 'ZipArchive::ER_REMOVE', 'message' => "Can't remove file"],
            23 => ['name' => 'ZipArchive::ER_DELETED', 'message' => 'Entry has been deleted'],
        ];
        if (isset($errors[$p_error_no])) {
            return $errors[$p_error_no]['message'];
        } else {
