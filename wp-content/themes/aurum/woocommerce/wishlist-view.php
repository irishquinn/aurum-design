...
<form id="yith-wcwl-form" action="<?php echo esc_url(YITH_WCWL()->get_wishlist_url('view' . ($wishlist_meta['is_default'] != 1 ? '/' . $wishlist_meta['wishlist_token'] : ''))) ?>" method="post" class="woocommerce <?php if (count($wishlist_items) == 0) {
    echo 'empty';
} ?>">
						...
						<tr id="yith-wcwl-row-<?php echo esc_attr($item['prod_id']); ?>" data-row-id="<?php echo esc_attr($item['prod_id']); ?>" class="<?php echo esc_attr($stock_status); ?>">
							...
							<td class="product-add-to-cart pr" data-title="<?php esc_attr_e('Add To Cart', 'wr-nitro'); ?>">
								...
								<?php if ($show_add_to_cart && isset($stock_status) && $stock_status != 'Out'): ?>
									<?php
                                        echo apply_filters(
                                    'woocommerce_wishlist_add_to_cart_link',
                                    sprintf(
                                                '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="' . (($product->is_type('simple')) ? 'ajax_add_to_cart' : '') . ' product__btn_cart product__btn btr-50 button %s product_type_%s"><i class="nitro-icon-' . esc_attr($icons) . '-cart"></i>%s</a>',
					...
					<div class="fc jcsb mgb10">
						<?php if ($show_cb) : ?>
							<div class="custom-add-to-cart-button-cotaniner">
								<a href="<?php echo esc_url(add_query_arg(['wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token']])) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters('yith_wcwl_custom_add_to_cart_text', esc_html__('Add the selected products to the cart', 'wr-nitro')) ?></a>
							</div>
						<?php endif; ?>
