...
<div id="product-<?php the_ID(); ?>" <?php post_class('style-5 social-circle ' . $layout . (is_customize_preview() ? ' customizable customize-section-product_single' : '')); ?>>
	<div class="oh pdt30 <?php if ('no-sidebar' != $layout) {
    echo 'container';
} ?>">
		<div id="shop-detail">
			<div class="container">
					...
					<div class="w667-12 cm-4">
						<div class="p-single-info">
												...
												<?php
                                                    if (version_compare(WC_VERSION, '3.0.0', '<')) {
                                                        if ($product->manage_stock == 'yes' && !empty($availability['availability'])) :
                                                            echo esc_html($availability['availability']); elseif ($product->manage_stock == 'no' && $product->is_in_stock()) :
                                                            esc_html_e('In Stock', 'wr-nitro'); else :
                                                            esc_html_e('Out Of Stock', 'wr-nitro');
                                                        endif;
                                                    } else {
                                                        if ($product->get_manage_stock() && !empty($availability['availability'])) :
                                                            echo esc_html($availability['availability']); elseif (!$product->get_manage_stock() && $product->is_in_stock()) :
                                                            esc_html_e('In Stock', 'wr-nitro'); else :
                                                            esc_html_e('Out Of Stock', 'wr-nitro');
                                                        endif;
                                                    }
							...
							<?php
                                ...
                                remove_action('woocommerce_single_product_summary', ['WR_Nitro_Pluggable_WooCommerce', 'nitro_template_single_share'], 50);
			...
			<div class="addition-product <?php if ($wr_nitro_options['wc_single_product_related_full'] == 'boxed') {
                                echo 'container';
                            } ?>">
