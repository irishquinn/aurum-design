...
if (function_exists('is_woocommerce') && is_woocommerce() || function_exists('is_account_page') && is_account_page() || function_exists('is_cart') && is_cart() || function_exists('is_checkout') && is_checkout() || function_exists('yith_wcwl_is_wishlist') && yith_wcwl_is_wishlist_page()) {
    return;
}
...
<div class="primary-sidebar <?php if ($wr_sticky === true) {
    echo 'primary-sidebar-sticky';
} ?> widget-style-<?php echo esc_attr($wr_style) . ' ' . (is_customize_preview() ? 'customizable customize-section-widget_styles ' . $wr_sidebar_border : ''); ?>" <?php WR_Nitro_Helper::schema_metadata(['context' => 'sidebar']); ?>>

	<?php if (is_active_sidebar($wr_sidebar)) : ?>

		<div class="mgt30 <?php if ($wr_sticky === true) {
    echo 'primary-sidebar-inner';
} ?>">
